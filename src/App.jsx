...
import logo from '../Header-20260911T121624Z-1-001/Header/LOGO_S/LOGO_GEMS_INSIDER_DARK_THEME_CROPPED.png'
...
function App() {
  ...
  useEffect(() => {
    const unsubscribe = onAuthStateChanged(auth, (account) => {
      ...
      setUser({ uid: account.uid, email: account.email, name: account.displayName || account.email, role: account.email === 'admin@gemsinsider.com' ? 'admin' : 'user' })
    })
    return unsubscribe
  }, [])
  ...
  const submitAuth = async (event) => {
    ...
    try {
      if (authMode === 'register') {
        ...
        const role = email === 'admin@gemsinsider.com' ? 'admin' : 'user'
        await setDoc(doc(db, 'users', account.user.uid), { email, name, role, createdAt: serverTimestamp() })
      } else {
        await signInWithEmailAndPassword(auth, email, password)
      }
    } catch (error) {
      setAuthError(error.code === 'auth/email-already-in-use' ? 'An account with this email already exists.' : 'Email or password is incorrect.')
      return
    }
    setAuthOpen(false)
  ...
  const submitDemoPayment = (event) => {
    event.preventDefault()
    setPaymentError('Payment could not be completed. Please verify your card details and try again.')
  }
    ...
    <main className={theme === 'light' ? 'theme-light' : ''}>
      ...
      {paymentOpen && <div className="overlay" role="presentation" onClick={() => setPaymentOpen(false)}>
        <div className="auth-modal payment-modal" role="dialog" aria-label="Card payment" onClick={(event) => event.stopPropagation()}>
          ...
          <p className="auth-note">{pendingBuy.length} stone{pendingBuy.length === 1 ? '' : 's'} selected. This demo form does not process or store card details.</p>
          <form onSubmit={submitDemoPayment} className="auth-form">
            ...
            <label><span>Card number</span><input name="cardNumber" type="text" inputMode="numeric" autoComplete="cc-number" placeholder="0000 0000 0000 0000" maxLength="19" required /></label>
            <div className="payment-fields">
              <label><span>Expiry</span><input name="expiry" type="text" inputMode="numeric" autoComplete="cc-exp" placeholder="MM / YY" required /></label>
              <label><span>CVV</span><input name="cvv" type="password" inputMode="numeric" autoComplete="cc-csc" maxLength="4" required /></label>
            </div>
